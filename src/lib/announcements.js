...
export async function saveBibleVerse(verse) {
  if (verse.id) {
    ...
    await logCmsAudit({
      action: 'updated', module: 'announcements', entityType: 'bible_verses',
      entityId: verse.id, entityLabel: verse.verse_reference || verse.id,
      summary: `Updated bible verse ${verse.verse_reference || verse.id}`,
    })
  } else {
    const { error } = await supabase.from('bible_verses').insert(verse)
    if (error) throw error
    await logCmsAudit({
      action: 'created', module: 'announcements', entityType: 'bible_verses',
      entityLabel: verse.verse_reference || 'verse',
      summary: `Added bible verse ${verse.verse_reference || ''}`.trim(),
    })
  }
}
...
export async function deleteBibleVerse(id) {
  ...
  if (error) throw error
  await logCmsAudit({
    action: 'deleted', module: 'announcements', entityType: 'bible_verses',
    entityId: id, summary: `Deleted bible verse ${id}`,
  })
}
...
export async function toggleVerseActive(id, is_active) {
  ...
  await logCmsAudit({
    ...
    module: 'announcements', entityType: 'bible_verses', entityId: id,
    summary: `${is_active ? 'Activated' : 'Deactivated'} bible verse ${id}`,
  })
}
...
export async function bulkUpsertVerses(verses) {
  ...
  if (error) throw error
  await logCmsAudit({
    action: 'saved', module: 'announcements', entityType: 'bible_verses',
    entityLabel: `${verses.length} verses`,
    summary: `Bulk upserted ${verses.length} bible verse(s)`,
  })
...
export async function saveAnnouncementSettings(settings, updatedBy) {
  ...
  await logCmsAudit({
    action: 'saved', module: 'announcements', entityType: 'announcement_settings',
    summary: 'Saved announcement settings',
...
export async function upsertExclusion({ member_id, member_name, family_id, exclusion_type, reason, added_by }) {
  ...
  if (error) throw error
  await logCmsAudit({
    action: 'saved', module: 'announcements', entityType: 'announcement_exclusions',
    entityId: member_id, entityLabel: member_name || member_id,
    summary: `Saved announcement exclusion for ${member_name || member_id} (${exclusion_type})`,
    actor: added_by ? { email: added_by } : null,
  })
}
...
export async function removeExclusion(id) {
  ...
  if (error) throw error
  await logCmsAudit({
    action: 'deleted', module: 'announcements', entityType: 'announcement_exclusions',
    entityId: id, summary: `Removed announcement exclusion ${id}`,
  })
}
