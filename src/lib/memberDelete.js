...
export async function restoreMember(deletedMemberId, newMemberId, userEmail, restoreReason) {
  try {
    ...
    // Capture original member_id BEFORE restore removes the deleted row
    const originalMemberId = await getOriginalMemberId(deletedMemberId)
    ...
    // 2. Move photo back — always from the ORIGINAL deleted path
    const memberIdToUse = newMemberId || originalMemberId
    if (originalMemberId) {
      await movePhotoToActive(originalMemberId, memberIdToUse)
    }
 ...
 * @param {string} fromMemberId — ID under which the photo was archived (deleted/{id}.jpg)
 * @param {string} [toMemberId] — active ID to restore into (defaults to fromMemberId)
 */
async function movePhotoToActive(fromMemberId, toMemberId = fromMemberId) {
  try {
    const deletedPath = `deleted/${fromMemberId}.jpg`
    const activePath  = `active/${toMemberId}.jpg`

    // Check if photo exists in deleted folder
    const { data: fileExists } = await supabase.storage
      .from('member-photos')
      .list('deleted', { search: `${fromMemberId}.jpg` })
    ...
    const { data: activeExists } = await supabase.storage
      .from('member-photos')
      .list('active', { search: `${toMemberId}.jpg` })
    ...
    console.warn(`Could not move photo back for ${fromMemberId} → ${toMemberId}:`, err.message)
    return { success: false, moved: false, error: err.message }
  }
}
