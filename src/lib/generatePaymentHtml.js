...
export async function generateAndUploadPaymentHtml({ req, church, catMap }) {
  ...
  const upiContent = `upi://pay?pa=${encodeURIComponent(upiId)}&pn=${encodeURIComponent(churchName)}&cu=INR`
  const gpayUrl    = `gpay://upi/pay?pa=${encodeURIComponent(upiId)}&pn=${encodeURIComponent(churchName)}&cu=INR`
...
function openGPay(){
  var id='${jsStr(upiId)}',pn=encodeURIComponent('${jsStr(churchName)}');
