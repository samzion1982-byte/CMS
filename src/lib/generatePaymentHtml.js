...
export async function generateAndUploadPaymentHtml({ req, church, catMap }) {
  ...
  const upiContent = `upi://pay?pa=${encodeURIComponent(upiId)}&pn=${encodeURIComponent(churchName)}&am=${total}&cu=INR&tn=ChurchOffering`
  const gpayUrl    = `gpay://upi/pay?pa=${encodeURIComponent(upiId)}&pn=${encodeURIComponent(churchName)}&am=${total}&cu=INR&tn=ChurchOffering`
