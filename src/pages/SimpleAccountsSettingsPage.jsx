...
export default function SimpleAccountsSettingsPage() {
  ...
  async function tryUnlock() {
    const ok = await verifyMasterPassword(unlockInput)
    if (ok) {
  ...
  async function handleFlush() {
    const ok = await verifyMasterPassword(flushPwd)
    if (!ok) { setFlushErr('Incorrect master password'); return }
    setFlushing(true)
    try {
  ...
  async function handleRestore() {
    const ok = await verifyMasterPassword(restPwd)
    if (!ok) { setRestErr('Incorrect master password'); return }
