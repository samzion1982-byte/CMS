-- RPC functions to flush accounting data
-- Called from Church Setup → Accounts Module → Flush Accounts

-- Advanced Accounts flush: wipes all entities, COA, journal entries, resets method lock, re-seeds COA
CREATE OR REPLACE FUNCTION flush_accounting_data()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  TRUNCATE
    accounting_audit_log,
    account_balances,
    journal_entry_lines,
    journal_entries,
    bank_accounts,
    chart_of_accounts,
    accounting_entities
  RESTART IDENTITY CASCADE;

  UPDATE churches
  SET accounting_entry_system_locked = false,
      accounting_entry_system        = 'double'
  WHERE id IS NOT NULL;

  -- Re-seed default Chart of Accounts (3-level hierarchy)
  -- Level 1 — Main Groups
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, sort_order) VALUES
    ('A',   'Assets',           'Asset',     1, false, 10),
    ('L',   'Liabilities',      'Liability', 1, false, 20),
    ('EQ',  'Equity & Funds',   'Equity',    1, false, 30),
    ('INC', 'Income',           'Income',    1, false, 40),
    ('EXP', 'Expenses',         'Expense',   1, false, 50)
  ON CONFLICT (code) DO NOTHING;

  -- Level 2 — Account Groups
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Asset', 2, false, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('A-CA',  'Current Assets',    10),
    ('A-FA',  'Fixed Assets',      20),
    ('A-INV', 'Investments',       30),
    ('A-ADV', 'Advances & Loans',  40)
  ) AS v(code, name, so) WHERE p.code = 'A' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Liability', 2, false, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('L-CL', 'Current Liabilities',   10),
    ('L-LL', 'Long-term Liabilities', 20)
  ) AS v(code, name, so) WHERE p.code = 'L' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Equity', 2, false, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('EQ-CF', 'Church Funds',  10),
    ('EQ-RS', 'Reserves',      20)
  ) AS v(code, name, so) WHERE p.code = 'EQ' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Income', 2, false, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('INC-CH', 'Church Collections', 10),
    ('INC-OT', 'Other Income',       20)
  ) AS v(code, name, so) WHERE p.code = 'INC' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Expense', 2, false, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('EXP-ST', 'Staff & Clergy',       10),
    ('EXP-AD', 'Administration',       20),
    ('EXP-MN', 'Ministry & Outreach',  30),
    ('EXP-MT', 'Maintenance',          40)
  ) AS v(code, name, so) WHERE p.code = 'EXP' ON CONFLICT (code) DO NOTHING;

  -- Level 3 — Ledgers
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Asset', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('A-CA-001', 'Cash in Hand',            10),
    ('A-CA-002', 'Petty Cash',              20),
    ('A-CA-003', 'Bank - Current Account',  30),
    ('A-CA-004', 'Bank - Savings Account',  40)
  ) AS v(code, name, so) WHERE p.code = 'A-CA' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Asset', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('A-FA-001', 'Building / Church Property', 10),
    ('A-FA-002', 'Furniture & Equipment',      20),
    ('A-FA-003', 'Musical Instruments',        30),
    ('A-FA-004', 'Vehicles',                   40)
  ) AS v(code, name, so) WHERE p.code = 'A-FA' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Asset', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('A-INV-001', 'Fixed Deposits',      10),
    ('A-INV-002', 'Recurring Deposits',  20)
  ) AS v(code, name, so) WHERE p.code = 'A-INV' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Asset', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('A-ADV-001', 'Advance to Staff',    10),
    ('A-ADV-002', 'Advance to Members',  20)
  ) AS v(code, name, so) WHERE p.code = 'A-ADV' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Liability', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('L-CL-001', 'Accounts Payable',      10),
    ('L-CL-002', 'Advance from Members',  20),
    ('L-CL-003', 'Dues & Payables',       30)
  ) AS v(code, name, so) WHERE p.code = 'L-CL' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Liability', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('L-LL-001', 'Loan from Bank',    10),
    ('L-LL-002', 'Loan from Members', 20)
  ) AS v(code, name, so) WHERE p.code = 'L-LL' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Equity', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('EQ-CF-001', 'General Fund',      10),
    ('EQ-CF-002', 'Building Fund',     20),
    ('EQ-CF-003', 'Mission Fund',      30),
    ('EQ-CF-004', 'Education Fund',    40),
    ('EQ-CF-005', 'Benevolence Fund',  50)
  ) AS v(code, name, so) WHERE p.code = 'EQ-CF' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Equity', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('EQ-RS-001', 'Opening Balance Reserve',  10),
    ('EQ-RS-002', 'Surplus / Deficit',        20)
  ) AS v(code, name, so) WHERE p.code = 'EQ-RS' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Income', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('INC-CH-001', 'Sunday Offerings',             10),
    ('INC-CH-002', 'Tithe / Subscription',         20),
    ('INC-CH-003', 'Special Offerings',            30),
    ('INC-CH-004', 'Harvest Festival',             40),
    ('INC-CH-005', 'Christmas / Easter Offerings', 50)
  ) AS v(code, name, so) WHERE p.code = 'INC-CH' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Income', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('INC-OT-001', 'Donations',       10),
    ('INC-OT-002', 'Interest Income', 20),
    ('INC-OT-003', 'Rent Income',     30),
    ('INC-OT-004', 'Other Income',    40)
  ) AS v(code, name, so) WHERE p.code = 'INC-OT' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Expense', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('EXP-ST-001', 'Pastoral Salary',      10),
    ('EXP-ST-002', 'Staff Salaries',       20),
    ('EXP-ST-003', 'Provident Fund',       30),
    ('EXP-ST-004', 'Staff Medical',        40),
    ('EXP-ST-005', 'Pastoral Allowances',  50)
  ) AS v(code, name, so) WHERE p.code = 'EXP-ST' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Expense', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('EXP-AD-001', 'Stationery & Printing', 10),
    ('EXP-AD-002', 'Postage & Courier',     20),
    ('EXP-AD-003', 'Bank Charges',          30),
    ('EXP-AD-004', 'Audit Fees',            40),
    ('EXP-AD-005', 'Miscellaneous',         50)
  ) AS v(code, name, so) WHERE p.code = 'EXP-AD' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Expense', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('EXP-MN-001', 'Mission & Evangelism',  10),
    ('EXP-MN-002', 'Sunday School',         20),
    ('EXP-MN-003', 'Youth Ministry',        30),
    ('EXP-MN-004', 'Women Fellowship',      40),
    ('EXP-MN-005', 'Special Programmes',    50),
    ('EXP-MN-006', 'Medical / Benevolence', 60)
  ) AS v(code, name, so) WHERE p.code = 'EXP-MN' ON CONFLICT (code) DO NOTHING;

  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order)
  SELECT v.code, v.name, 'Expense', 3, true, p.id, v.so
  FROM chart_of_accounts p, (VALUES
    ('EXP-MT-001', 'Electricity & Water',  10),
    ('EXP-MT-002', 'Building Maintenance', 20),
    ('EXP-MT-003', 'Furniture Repairs',    30),
    ('EXP-MT-004', 'Vehicle Maintenance',  40),
    ('EXP-MT-005', 'Travel & Conveyance',  50)
  ) AS v(code, name, so) WHERE p.code = 'EXP-MT' ON CONFLICT (code) DO NOTHING;
END;
$$;

-- Simple Accounts flush: wipes transactions/accounts/categories, re-seeds defaults
CREATE OR REPLACE FUNCTION flush_simple_accounts()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  TRUNCATE simple_transactions, simple_accounts, simple_categories
  RESTART IDENTITY CASCADE;

  -- Re-seed default categories
  INSERT INTO simple_categories (name, type, is_default, sort_order) VALUES
    ('Sunday Offering',       'income',  true, 10),
    ('Tithes',                'income',  true, 20),
    ('Special Offering',      'income',  true, 30),
    ('Donations',             'income',  true, 40),
    ('Events & Programs',     'income',  true, 50),
    ('Other Income',          'income',  true, 60),
    ('Salaries & Honorarium', 'expense', true, 10),
    ('Rent & Utilities',      'expense', true, 20),
    ('Maintenance & Repairs', 'expense', true, 30),
    ('Events & Programs',     'expense', true, 40),
    ('Stationery & Printing', 'expense', true, 50),
    ('Travel & Transport',    'expense', true, 60),
    ('Miscellaneous',         'expense', true, 70);

  -- Re-seed default accounts
  INSERT INTO simple_accounts (name, account_type, sort_order) VALUES
    ('Cash', 'cash', 10),
    ('Bank', 'bank', 20);
END;
$$;
